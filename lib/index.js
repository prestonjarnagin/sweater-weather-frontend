...
$('nav').on('click', '.delete-city-btn', function(self){
  // TODO This is sort of gross
  let deletedCity = self.currentTarget.parentElement.innerText.slice(0,-1)
  deleteCity(deletedCity)
  return false;
})

function deleteCity(deletedCity) {

}

function addNewFavorite(city){
  let newCityPath = '/api/v1/favorites'
  fetch(`${SERVER_PATH}${newCityPath}`, {
    method: 'POST',
  ...
  fetch(`${SERVER_PATH}${favoritesPath}`, {
   ...
   .then(function(reply){
     ...
     reply.data.forEach(function(element){
      ...
      $(favorites).append(`
        <span><a class="dropdown-item" href="javascript: populateFromFavorite('${city}')">${city}<button class="btn btn-outline-danger delete-city-btn">X</button></a>
      `)
