...
$(document).ready(function(){
  populateWeather(env.DEFAULT_CITY);
  $("#weather-location-query").attr("placeholder", `${env.DEFAULT_CITY}`);
})
...
function sendRequest(method, path, body){
  return fetch(`${env.SERVER_PATH}${path}`, {
    method: method,
...
function populateWeather(query){
  ...
  oReq.open("GET", `${env.SERVER_PATH}/api/v1/forecast${params}`);
  oReq.send();
}
...
function displayUserButtons(){
  ...
  function getUserFavorites(){
    ...
    fetch(`${env.SERVER_PATH}${favoritesPath}`, {
      method: 'GET',
      headers: {'Content-Type': 'application/json',
      'Accept': 'application/json',
      'api_key': sessionStorage.getItem('key')
  ...
  function buildWeatherHour(element){
    let time = dateTime.parseTime(element.time)
  ...
  function buildWeatherDay(element){
    ...
    let day =  `Date: ${dateTime.parseDate(element.time)}
